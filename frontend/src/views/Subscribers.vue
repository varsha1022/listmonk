  ...
  <section class="subscribers">
    ...
    <b-table :data="subscribers.results ?? []" :loading="loading.subscribers" @check-all="onTableCheck"
      ...
      <b-table-column v-slot="props" field="email" :label="$t('subscribers.email')" header-class="cy-email" sortable
        :td-attrs="$utils.tdID">
        <a :href="'/subscribers/' + encodeURIComponent(String(props.row.id))" @click.prevent="showEditForm(props.row)"
      ...
      <b-table-column v-slot="props" cell-class="actions" align="right">
        <div>
          <a :href="'/api/subscribers/' + encodeURIComponent(String(props.row.id)) + '/export'" data-cy="btn-download"
            :aria-label="$t('subscribers.downloadData')">
            <b-tooltip :label="$t('subscribers.downloadData')" type="is-dark">
              <b-icon icon="cloud-download-outline" size="is-small" />
            </b-tooltip>
          </a>
          <a v-if="$can('subscribers:manage')" :href="'/subscribers/' + encodeURIComponent(String(props.row.id))"
